...
export default async function EmployerSearchPage({
  ...
  return (
    <div className="grid gap-6">
      ...
      />
      {!access.ok ? (
        <section className="rounded-lg border border-[#efc2d2] bg-[#fff4f7] p-5">
          <h2 className="text-lg font-semibold text-[#201925]">Complete these details before searching candidates</h2>
          <p className="mt-2 text-sm text-[#66616f]">{access.message}</p>
          <ButtonLink href="/employer/onboarding" className="mt-4">Complete setup</ButtonLink>
        </section>
      ) : access.warning ? (
