          ...
          <div className="mx-auto grid max-w-7xl gap-8 px-4 py-10 sm:px-6 sm:py-12 lg:grid-cols-[.95fr_1.05fr] lg:items-center lg:gap-10 lg:px-8 lg:py-14">
            <div className="min-w-0 max-w-xl text-left">
              ...
              <div className="kaam-trust-marquee mt-7 min-w-0 overflow-hidden border-t border-[#f1dce5] pt-5 sm:mt-8 sm:pt-6" aria-label="KAAM trust benefits">
                ...
                <div className="hidden min-w-0 grid-cols-3 gap-4 lg:grid">
                  {trustBenefits.map(([icon, title, text]) => <div className="flex min-w-0 items-start gap-2" key={title}><span className="shrink-0 text-xl text-[#f56ba1]">{icon}</span><div className="min-w-0"><p className="text-sm font-bold leading-5 text-[#160847]">{title}</p><p className="mt-0.5 text-xs leading-4 text-[#766b74]">{text}</p></div></div>)}
                </div>
              </div>
            </div>
