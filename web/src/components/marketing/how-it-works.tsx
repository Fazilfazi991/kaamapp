...
export function HowItWorks() {
  ...
  return (
    <section className="relative overflow-hidden bg-gradient-to-b from-white via-[#fcfbff] to-[#f6f3ff] py-8 sm:py-9" aria-labelledby="how-it-works-heading">
      <div className="pointer-events-none absolute left-5 top-28 hidden h-4 w-4 rotate-45 bg-[#b7a8e6] lg:block" />
      <div className="pointer-events-none absolute right-12 top-44 hidden text-4xl text-[#8d7aca] lg:block">✦</div>
      <div className="mx-auto max-w-7xl px-[5%]">
        ...
        <div className="lg:hidden">
          ...
          <div className="mx-auto mt-7 max-w-xl">
            ...
            <div className="relative space-y-4 before:absolute before:bottom-7 before:left-7 before:top-7 before:border-l-2 before:border-dashed before:border-[#cfc4f1]">
        ...
        <div className="relative z-10 mt-3 grid gap-2 rounded-2xl border border-[#e5dff4] bg-white/95 p-3 shadow-[0_8px_24px_rgba(22,8,71,.08)] sm:grid-cols-2 lg:grid-cols-4 lg:gap-0">
          {benefits.map((benefit, index) => <div key={benefit.title} className={`flex items-center gap-2 px-3 py-2.5 ${index > 0 ? "lg:border-l lg:border-[#eee9ff]" : ""}`}><span className="grid h-11 w-11 shrink-0 place-items-center rounded-full bg-[#f0edfb] text-[#160847]"><LineIcon name={benefit.icon} size={23} /></span><div><h3 className="text-sm font-bold text-[#160847]">{benefit.title}</h3><p className="mt-0.5 text-xs leading-4 text-[#706578]">{benefit.text}</p></div></div>)}
        </div>
      </div>
    </section>
  );
...
function Journey({ title, subtitle, steps, side }: { title: string; subtitle: string; steps: Step[]; side: "left" | "right" }) {
  return <div><div className={side === "right" ? "pl-2" : "pr-2"}><h3 className="text-2xl font-bold tracking-tight text-[#160847] xl:text-3xl">{title}</h3><span className="mt-2 block h-1 w-11 rounded-full bg-[#160847]" /><p className="mt-2 text-base text-[#6d6376]">{subtitle}</p></div><div className="relative mt-4 space-y-3 before:absolute before:bottom-6 before:left-7 before:top-6 before:border-l-2 before:border-dashed before:border-[#cfc4f1]">{steps.map((step,index) => <StepCard key={step.title} step={step} index={index} />)}</div></div>;
}

function StepCard({ step, index }: { step: Step; index: number }) {
  return <article className="relative z-10 grid min-h-[94px] grid-cols-[52px_minmax(0,1fr)_42px] items-center gap-2 rounded-2xl border border-[#e7e1f2] bg-white px-3 py-2.5 shadow-[0_5px_16px_rgba(22,8,71,.07)] transition duration-200 hover:-translate-y-0.5 hover:border-[#d6ccef] hover:shadow-[0_10px_24px_rgba(22,8,71,.11)] sm:grid-cols-[64px_minmax(0,1fr)_54px]"><span className="relative grid h-11 w-11 place-items-center rounded-full bg-gradient-to-br from-[#28115f] to-[#160847] text-lg font-bold text-white ring-3 ring-[#f3f0ff] before:absolute before:-inset-1.5 before:rounded-full before:border before:border-dashed before:border-[#b9ace2] before:content-[''] sm:h-13 sm:w-13 sm:text-xl">{index + 1}</span><div className="min-w-0 self-center"><h4 className="text-[15px] font-bold leading-5 text-[#160847] sm:text-base">{step.title}</h4><p className="mt-0.5 text-sm leading-5 text-[#6d6376]">{step.description}</p></div><span className="grid h-10 w-10 place-items-center rounded-xl bg-[#f1effb] text-[#160847] sm:h-12 sm:w-12"><LineIcon name={step.icon} size={22} /></span></article>;
}

function Hub() {
  return <div className="relative flex min-h-[470px] items-center justify-center"><svg className="absolute inset-0 h-full w-full" viewBox="0 0 400 540" fill="none" aria-hidden="true"><g stroke="#160847" strokeWidth="2"><path d="M0 110h55c26 0 20 140 70 140h20"/><path d="M0 230h55c26 0 20 20 70 20h20"/><path d="M0 350h55c26 0 20-100 70-100h20"/><path d="M0 470h55c26 0 20-220 70-220h20"/><path d="M400 110h-55c-26 0-20 140-70 140h-20"/><path d="M400 230h-55c-26 0-20 20-70 20h-20"/><path d="M400 350h-55c-26 0-20-100-70-100h-20"/><path d="M400 470h-55c-26 0-20-220-70-220h-20"/></g><g fill="#160847">{[110,230,350,470].flatMap((cy) => [[0,cy],[400,cy]]).map(([cx,cy])=><circle key={`${cx}-${cy}`} cx={cx} cy={cy} r="4"/>)}<circle cx="145" cy="250" r="6" fill="white" stroke="#160847" strokeWidth="2"/><circle cx="255" cy="250" r="6" fill="white" stroke="#160847" strokeWidth="2"/></g></svg><div className="relative z-10 flex h-40 w-40 items-center justify-center rounded-full bg-[#160847] p-8 shadow-[0_0_0_12px_rgba(22,8,71,.08),0_0_0_28px_rgba(117,98,184,.07),0_16px_32px_rgba(22,8,71,.22)] ring-2 ring-dashed ring-[#b9ace2]"><Image src="/kaam-logo.webp" alt="KAAM" width={120} height={48} className="h-auto w-full" /></div><div className="absolute top-3 text-[#160847]"><LineIcon name="trophy" size={46} /></div><div className="absolute bottom-1 rounded-full border border-[#e5dff4] bg-white px-3 py-2 text-xs font-semibold text-[#302a39] shadow-sm">✓ Real people. Real opportunities.</div><div className="absolute right-7 top-14 text-base text-[#8d7aca]">✦</div><div className="absolute bottom-14 left-7 text-[#7562b8]"><LineIcon name="send" size={25} /></div></div>;
}
