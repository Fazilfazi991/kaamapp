            ...
            {account.authenticated && account.dashboardHref ? (
              <div className="grid gap-3">
                ...
                <nav className="grid gap-1" aria-label="Account navigation">
                  {accountLinks.map((item) => <Link key={item.href} href={item.href} prefetch={false} className="rounded-lg px-3 py-3 text-sm font-semibold text-[#201925] hover:bg-[#f7f4ff]">{item.label}</Link>)}
                </nav>
