    ...
    <header className="border-b border-[#160847] bg-[#160847] text-white">
      <div className="mx-auto flex max-w-6xl items-center justify-between gap-4 px-4 py-4 sm:px-6 lg:px-8">
        ...
        <nav className="hidden items-center gap-1 md:flex" aria-label="Primary">
          ...
          {account.authenticated && account.dashboardHref ? <><Link href={account.dashboardHref} className="focus-ring flex items-center gap-2 rounded-lg px-2 py-1.5 hover:bg-white/10"><span aria-hidden="true" className="grid h-9 w-9 place-items-center rounded-full border border-white/40 bg-white/10 text-xs font-bold text-white">{initials}</span><span className="max-w-28 truncate text-sm font-semibold text-white">Hi, {firstName}</span></Link><ButtonLink href={account.dashboardHref} className="min-h-10 bg-[#f56ba1] px-4 py-2 text-[#160847] hover:bg-[#f889b6]">{dashboardLabel}</ButtonLink><form action={signOutToHomeAction}><button type="submit" className="focus-ring rounded-md px-2 py-2 text-sm font-medium text-white/90 hover:text-[#f56ba1]">Logout</button></form></> : <><ButtonLink href={routes.login} className="min-h-10 border border-white/60 bg-transparent px-4 py-2 text-white hover:bg-white/10">Login</ButtonLink><ButtonLink href={routes.register} className="min-h-10 bg-[#f56ba1] px-4 py-2 text-[#160847] hover:bg-[#f889b6]">Get started</ButtonLink></>}
        </nav>
