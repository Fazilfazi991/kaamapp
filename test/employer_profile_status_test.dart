...
void main() {
  group('employer company profile status payload', () {
    ...
    setUpAll(() {
      ...
      final candidateStart = backendSource.indexOf(
        'class CandidateProfileRepository',
      );
      candidateRepositorySource = backendSource.substring(
        candidateStart,
        employerStart,
      );
    ...
    test('live profile_status enum values exclude pending', () {
      expect(KaamProfileStatus.liveEnumValues, {
        'draft',
        'active',
        'paused',
        'blocked',
    ...
    test(
      'candidate onboarding still uses a valid enum-backed profile status',
      () {
    ...
    test(
      'employer onboarding completes without a verification document gate',
      () {
        expect(
          onboardingSource,
          contains('AppRoutes.employerProfileComplete'),
        );
        expect(onboardingSource, isNot(contains('AppRoutes.dashboard')));
      },
    );

    test(
      'invalid database details are mapped to a safe company save message',
      () {
        final message = KaamSafeErrorMessages.employerCompanySaveMessage(
          StateError('invalid input value for enum profile_status: "pending"'),
        );
