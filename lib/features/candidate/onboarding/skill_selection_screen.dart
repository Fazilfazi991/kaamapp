        ...
        for (final skill in value.skills) ...[
          AppCard(
            ...
            child: CheckboxListTile(
              ...
              onChanged: (_) => setState(() => value.primarySkillId = skill.id),
            ),
          ),
          const SizedBox(height: 8),
        ],
...
class _SkillDetailsScreenState extends State<SkillDetailsScreen> {
  ...
  void didChangeDependencies() {
    ...
    if (draft != null && details.isEmpty) {
      for (final skill in draft!.skills) {
        details[skill.id] = _SkillDetail.fromSaved(
          draft!.savedBySkillId[skill.id],
        );
      }
    }
  }
  ...
  Future<void> _save() async {
    ...
    if (value.skills.any(
      (skill) =>
          !CandidateSkillExperience.isValid(details[skill.id]!.experience),
    )) {
      _message('Add experience for every selected skill.');
      return;
    }
    if (primary.level.isEmpty) {
      _message('Add experience and skill level for your main profession.');
      return;
    }
    setState(() => saving = true);
    try {
      await repository.saveSkills(
        selections: [
          for (final skill in value.skills)
            CandidateSkillData(
              skill: skill,
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('Your skills were saved.')));
      ...
      if (mounted) {
        _message('We could not save your job preferences. Please try again.');
      }
    } finally {
  ...
  Widget build(BuildContext context) {
    ...
    return ScreenScaffold(
      ...
      children: [
        ...
        const SizedBox(height: 8),
        const Text(
          'Add details for each job you selected.',
          style: AppTextStyles.body,
        ),
        const SizedBox(height: 16),
        for (final skill in value.skills) ...[
...
class _SkillDetail {
  ...
  factory _SkillDetail.fromSaved(CandidateSkillData? saved) {
    if (saved == null) return _SkillDetail();
    return _SkillDetail()
      ..experience = CandidateSkillExperience.labelFor(saved.experienceRange)
      ..level = saved.skillLevel
      ..uaeExperience = saved.uaeExperienceRange
      ..availability = saved.availability
      ..certificates.addAll(saved.certificateTypes)
      ..otherCertificate = saved.otherCertificateName;
  }
...
class _SkillDetailCard extends StatelessWidget {
  ...
  static final experience = CandidateSkillExperience.labels;
  static const levels = [
    'Beginner',
    'Intermediate',
    'Experienced',
