...
class DocumentStatusService {
  ...
  static String normalized(
    ...
    // Review state is canonical. A rejected version can still have incomplete
    // front/back paths while it is being replaced, so path existence must not
    // erase an explicit action-required status.
    if (value == rejected || value == reuploadRequired) return value;
  ...
  static String label(
    ...
    return switch (normalized(status, uploaded: uploaded, expiry: expiry)) {
      ...
      pending || pendingVerification => 'Pending Review',
      _ => 'Pending Review',
    };
  }
  ...
  static IconData icon(
    ...
    return switch (normalized(status, uploaded: uploaded, expiry: expiry)) {
      approved || verified => Icons.verified_rounded,
  ...
  static List<String> reminderDates(String expiryDate) {
    ...
    return [90, 60, 30, 7]
        .map(
          (days) =>
              date.subtract(Duration(days: days)).toUtc().toIso8601String(),
        )
        .toList();
  }
  ...
  static List<DocumentTimelineEntry> timeline({
    ...
    final uploadedAt =
        isPassport ? identity.passportUploadedAt : identity.visaUploadedAt;
    final status = isPassport ? identity.passportStatus : identity.visaStatus;
    final version =
        isPassport ? identity.passportVersion : identity.visaVersion;
    final entries = <DocumentTimelineEntry>[];
    if (uploadedAt.isNotEmpty) {
      entries.add(DocumentTimelineEntry('Document uploaded', uploadedAt));
      if (identity.ocrCompleted) {
        entries.add(DocumentTimelineEntry('OCR completed', uploadedAt));
      }
      entries.add(
        DocumentTimelineEntry(
          'Profile updated',
          identity.updatedAt.isEmpty ? uploadedAt : identity.updatedAt,
        ),
      );
      entries.add(
        DocumentTimelineEntry(label(status, uploaded: true), uploadedAt),
      );
      entries.add(
        DocumentTimelineEntry('Latest version v$version', uploadedAt),
      );
    }
    for (final old in versions.where(
      (item) => item.documentType == type.name && !item.isActive,
    )) {
      entries.add(
        DocumentTimelineEntry(
          'Previous version v${old.versionNumber}',
          old.createdAt,
        ),
      );
    }
