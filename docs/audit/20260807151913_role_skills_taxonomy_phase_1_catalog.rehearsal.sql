...
with category_seed(industry_slug, slug, name, sort_order, roles) as (values
 ...
 ('food-beverage','quick-service','Quick Service',30,'Pizza Maker|Burger Maker|Sandwich Maker|Fried Chicken Cook|Snack Maker|Quick Service Counter Staff'),
 ('food-beverage','beverage','Beverage',40,'Juice Maker|Tea Maker|Karak Maker|Barista|Mocktail Maker'),
 ('food-beverage','front-of-house','Front of House',50,'Waiter / Server|Captain|Restaurant Supervisor|Restaurant Manager|Restaurant Cashier|Host / Hostess|Food Runner|Busser|Order Taker|Restaurant Counter Staff'),
 ...
 ('housekeeping','residential-housekeeping','Residential Housekeeping',10,'Housekeeper|Residential Housekeeping Supervisor|Residential Laundry Attendant|Linen Attendant'),
 ...
 ('construction-civil','concrete-formwork','Concrete & Formwork',30,'Shuttering Carpenter|Reinforcement Steel Fixer|Rebar Worker|Concrete Worker|Formwork Foreman'),
 ...
 ('manufacturing-factory','production','Production',10,'Production Worker|General Factory Helper|Assembly Worker|Food Production Worker|Production Supervisor'),
 ...
 ('sales-customer-service','field-sales','Field Sales',10,'Sales Executive|Field Sales Coordinator|Merchandising Promoter'),
 ...
 ('salon-beauty','beauty-wellness','Beauty & Wellness',20,'Beauty Therapist|Nail Technician|Manicurist|Pedicurist|Makeup Artist|Henna Artist|Spa Therapist|Massage Therapist|Eyelash Technician'),
 ('laundry','laundry-operations','Laundry Operations',10,'Laundry Worker|Laundry Attendant|Laundry Supervisor|Dry Cleaner|Garment Presser'),
 ('agriculture','farm-operations','Farm Operations',10,'Farm Worker|Gardener|Irrigation Technician|Nursery Worker|Greenhouse Worker|Agricultural Helper|Farm Supervisor'),
 ('landscaping','landscape-operations','Landscape Operations',10,'Landscaper|Landscape Gardener|Landscape Supervisor|Tree Trimmer|Irrigation Helper'),
 ('healthcare-support','patient-support','Patient Support',10,'Caregiver|Nursing Assistant|Patient Care Assistant|Home Care Assistant|Ward Assistant'),
 ('healthcare-support','clinic-support','Clinic Support',20,'Clinic Receptionist|Dental Assistant|Pharmacy Assistant|Medical Records Clerk|Clinic Assistant'),
 ('office-administration','office-support','Office Support',10,'Office Boy|Office Assistant|Data Entry Operator|Document Controller|Receptionist|Pantry Assistant'),
 ('office-administration','business-support','Business Support',20,'PRO Assistant|HR Assistant|Business Sales Coordinator|Business Customer Service Representative|Business Call Center Agent'),
 ...
 ('general-labour','general-support','General Support',10,'General Helper|Loading Worker|Packing Helper|Site Helper|Factory Helper|General Kitchen Helper'),
...
from category_seed c join inserted_categories jc on jc.slug=c.slug
...
do $$
begin
  if (select count(*) from public.industries where active) <> 32 then raise exception 'Phase 1 taxonomy expected 32 industries'; end if;
  if (select count(*) from public.job_categories where active) <> 68 then raise exception 'Phase 1 taxonomy expected 68 categories'; end if;
  if (select count(*) from public.job_roles where active) <> 394 then raise exception 'Phase 1 taxonomy expected 394 roles'; end if;
  if (select count(*) from public.competency_skills where active) <> 306 then raise exception 'Phase 1 taxonomy expected 306 competency skills'; end if;
  if (select count(*) from public.job_role_skills) <> 2039 then raise exception 'Phase 1 taxonomy expected 2039 role-skill mappings'; end if;
  if exists (select 1 from public.job_categories c left join public.industries i on i.id=c.industry_id where i.id is null) then raise exception 'Phase 1 taxonomy has orphan categories'; end if;
  if exists (select 1 from public.job_roles r left join public.job_categories c on c.id=r.category_id where c.id is null) then raise exception 'Phase 1 taxonomy has orphan roles'; end if;
  if exists (select 1 from public.job_role_aliases a left join public.job_roles r on r.id=a.job_role_id where r.id is null) then raise exception 'Phase 1 taxonomy has orphan aliases'; end if;
  if exists (select 1 from public.job_role_skills rs left join public.job_roles r on r.id=rs.job_role_id left join public.competency_skills s on s.id=rs.competency_skill_id where r.id is null or s.id is null) then raise exception 'Phase 1 taxonomy has orphan role-skill mappings'; end if;
end $$;

notify pgrst, 'reload schema';
select jsonb_build_object(
 'industries',(select count(*) from public.industries),
 'job_categories',(select count(*) from public.job_categories),
 'job_roles',(select count(*) from public.job_roles),
 'job_role_aliases',(select count(*) from public.job_role_aliases),
 'competency_skills',(select count(*) from public.competency_skills),
 'job_role_skills',(select count(*) from public.job_role_skills),
 'legacy_role_mappings',(select count(*) from public.legacy_role_mappings),
 'orphan_categories',(select count(*) from public.job_categories c left join public.industries i on i.id=c.industry_id where i.id is null),
 'orphan_roles',(select count(*) from public.job_roles r left join public.job_categories c on c.id=r.category_id where c.id is null),
 'orphan_aliases',(select count(*) from public.job_role_aliases a left join public.job_roles r on r.id=a.job_role_id where r.id is null),
 'orphan_role_skills',(select count(*) from public.job_role_skills rs left join public.job_roles r on r.id=rs.job_role_id left join public.competency_skills s on s.id=rs.competency_skill_id where r.id is null or s.id is null)
) as rehearsal_result;

with terms(term) as (values ('porotta'),('parotta'),('paratha'),('porot'),('ac tech'),('hvac'),('tile'),('tile fixer'),('cleaner'),('electrician'),('waiter'),('car mechanic'),('forklift'),('barista'),('shawarma'),('kitchen helper'),('ductman')),
searches as (
  select t.term, coalesce(jsonb_agg(jsonb_build_object('role',s.role_name,'category',s.category_name,'industry',s.industry_name,'alias',s.matched_alias) order by s.relevance desc), '[]'::jsonb) as results
  from terms t left join lateral public.search_job_roles(t.term, 3) s on true group by t.term
), skills as (
  select r.name, jsonb_agg(s.name order by rs.is_core desc, rs.sort_order) as values
  from public.job_roles r join public.job_role_skills rs on rs.job_role_id=r.id join public.competency_skills s on s.id=rs.competency_skill_id
  where r.name in ('Porotta Maker','Electrician','HVAC Technician','Waiter / Server','Auto Mechanic','Tile Mason','Barista','General Cleaner','Forklift Operator')
  group by r.name
)
select jsonb_build_object('searches',(select jsonb_object_agg(term,results) from searches),'skills',(select jsonb_object_agg(name,values) from skills)) as rehearsal_runtime;
rollback;
